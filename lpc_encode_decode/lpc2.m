...
% 尝试不同的比特数
bits_values = [4, 6, 8,10,16]; % 定义要尝试的比特数
% 比较重构信号和原信号
subplot(length(bits_values)+1,1,1);
plot(x); % 原始语音信号
...
for i=1:length(bits_values)
    bits=bits_values(i);
    reconstructed_signal = process_signal(x, Fs, bits);
    % 使用逻辑索引将大于1的值设置为1
    reconstructed_signal(reconstructed_signal > 0.4) = 0.4;
    
    % 使用逻辑索引将小于-1的值设置为-1
    reconstructed_signal(reconstructed_signal < -0.4) = -0.4;

    % 绘制重构信号图
    subplot(length(bits_values)+1, 1, i+1)
    plot(reconstructed_signal(1:size(x,1),:)); % 重构的语音信号
    title(['Reconstructed voiced frame with ' num2str(bits) ' bits']);
    xlabel('Sample');
    ylabel('Amplitude');
end
function reconstructed_signal=process_signal(x, Fs, bits)
    % 2. 分帧
    ...
    for i = 1:size(frames, 1)
        ...
        % 注入随机噪声
        noiseLevel = 0.1; % 定义噪声的相对电平
        noise = randn(size(decoded_residual1)) * noiseLevel;
        decoded_residual1_with_noise = decoded_residual1 + noise;
        % 通过LPC参数滤波重构语音信号
        re_with_noise = filter(1, decoded_A1, decoded_residual1_with_noise)';
        % 通过LPC参数滤波重构语音信号
    ...
    bps=int32(size(reconstructed_frames,1)*(size(reconstructed_frames,2)+p)*bits/1000);
    % 输出信噪比
    disp(['LPC SNR values(' num2str(bits) ' bit):' num2str(snr_values)]);
    disp(['数码率(' num2str(bits) ' bit):' num2str(bps) 'kbps']);
end
